const albumRandom = [
  {
    id: "mi05",
    nombre: "Currents",
    imagen: "https://upload.wikimedia.org/wikipedia/en/9/9b/Tame_Impala_-_Currents.png",
    like: true,
  },
];
/* --------------------------- listado de almbumes --------------------------- */
...
window.addEventListener("load", function () {
  ...
  const intervalo = setInterval(function () {
    ...
    footer.innerHTML = `<p>Plan:
    <span style="color:#1ed55f">
    <strong>${planesMensuales[posicion].tipo}</strong>
    $ ${planesMensuales[posicion].costo}</p><p>
    </span>
    ${planesMensuales[posicion].descripcion}</p>`;
...
function masMusica() {
  const myTimeout = setTimeout(preguntaConfirm, 12000);
  function preguntaConfirm(){

    let preguntaMasMusica = confirm("Hey");

    if (preguntaMasMusica) {
      window.open("https://open.spotify.com/")
    } else {

      const usuarioOficial = document.createElement("p");
      usuarioOficial.innerHTML = `<a>
          Usuario oficial de Spotifront
        </a>`;
      document.querySelector(".perfil").appendChild(usuarioOficial);
    }
  }
}
