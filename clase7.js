...
function mostrarDatosEnPerfil(listado) {
  // desarrollar la función 👇
  const albums = document.getElementById("cant-albums");
  const favs = document.getElementById("cant-favoritos");
  let albumsQty = 0;
  let FavsQty = 0;
  ...
  if (albumsQty == 1) {
    albums.innerText = `${albumsQty} álbum`
  } else {
    albums.innerHTML = `<strong>${albumsQty} álbumes</strong>`
  };
  if (FavsQty == 1) {
    favs.innerText = `${FavsQty} favorito`
  } else {
    favs.innerHTML = `<strong>${FavsQty} favoritos</strong>`
  };
...
renderizarAlbumes(albumRandom);
mostrarDatosEnPerfil(albumRandom);